...
const buyerOnboardingSteps: OnboardingStep[] = [
  {
    ...
    fields: [
      ...
      { name: 'location', type: 'text', label: 'Location', required: true, placeholder: 'City, State' },
      { name: 'experience', type: 'select', label: 'Buying Experience', required: true, options: ['first-time', 'experienced', 'serial'] },
      { name: 'timeline', type: 'select', label: 'Purchase Timeline', required: true, options: ['Immediately', 'Within 3 months', 'Within 6 months', 'Within 1 year', 'Over 1 year'] }
    ]
  },
  {
    id: 2,
    ...
    fields: [
      { name: 'investmentMin', type: 'number', label: 'Minimum Investment ($)', required: true, placeholder: '100000' },
      { name: 'investmentMax', type: 'number', label: 'Maximum Investment ($)', required: true, placeholder: '1000000' },
      { name: 'fundingSource', type: 'select', label: 'Funding Source', required: true, options: ['personal-savings', 'bank-loan', 'investors', 'combination'] },
      { name: 'preferredBusinessSize', type: 'select', label: 'Preferred Business Size', required: true, options: ['small', 'medium', 'large'] }
    ]
  },
  {
    id: 3,
    ...
    fields: [
      { name: 'industry', type: 'select', label: 'Primary Industry Interest', required: true, options: ['Technology', 'Healthcare', 'Retail', 'Manufacturing', 'Food & Beverage', 'Professional Services', 'Construction', 'Any'] },
      { name: 'bio', type: 'textarea', label: 'About You', required: true, placeholder: 'Tell sellers about your background, experience, and what you bring to the table...' }
    ]
  }
];
    ...
    <OnboardingLayout gradientColors={['#4facfe', '#00f2fe']}>
      ...
      <ButtonGroup>
        ...
        <Button onClick={handleNext} fullWidth={currentStep === 0}>
          {currentStep === buyerOnboardingSteps.length - 1 ? 'Complete Profile' : 'Continue'}
        </Button>
      </ButtonGroup>
    </OnboardingLayout>
  );
